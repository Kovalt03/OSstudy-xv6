...
found:
  ...
  np->tid = allocpid(); // 명세와 동일하게 맞추려면 pid 를 그냥 줌
  ...
  np->pagetable = main->pagetable;
  np->sz = main->sz;

  np->trapframe_va = TRAPFRAME - (PGSIZE * np->tid);
  if (mappages(np->pagetable, np->trapframe_va,
...
clone(void (*fcn)(void*, void*), void *arg1, void *arg2, void *stack)
{
  ...
  uint64 alignstack = (PGROUNDDOWN((uint64)stack));
  
  *(np->trapframe) = *(p->trapframe);
  np->trapframe->sp = alignstack + PGSIZE * 2;
  ...
  for (;;) {
    ...
    for(t = proc; t < &proc[NPROC]; t++){
      if(t!=p && t->main == p){
        ...
        if(t->state == ZOMBIE){
          tid = t->tid;
          if (stack != 0 && copyout(p->pagetable, (uint64)stack, (char *)&t->ustack, sizeof(void *)) < 0) {
            release(&t->lock);
            release(&wait_lock);
            return -1;
          }
...
{
  if(!t->is_thread) {
    printf("ERROR: freethread called by proc\n");
    return;
  }
  // printf("free : %ld\n", t->trapframe_va);
